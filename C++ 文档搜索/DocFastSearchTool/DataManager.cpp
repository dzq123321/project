...
//静态库
#pragma comment(lib, "./sqlite/sqlite3.lib")//设置link时的静态库
...
void DataManager::InsertDoc(const string &path, const string &doc)//插入检索到的这个文件
{
	char sql[SQL_BUFFER_SIZE] = {0};
...
void DataManager::GetDocs(const string &path, multiset<string> &docs)//获得表，进行对比
{
	char sql[SQL_BUFFER_SIZE] = {0};
...
void DataManager::DeleteDoc(const string &path, const string &doc)
{
	...
	///////////////////////////////////////////
	//级联删除目录下的子文件
	string doc_path = path; //C:\Users\baoso\Desktop\my_dir
	doc_path += "\\";       //C:\Users\baoso\Desktop\my_dir\ 
	doc_path += doc;        //C:\Users\baoso\Desktop\my_dir\CC
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//搜索的时候将汉字转换为拼音和首字母，随后进行模糊查询，将查询的结果（文件名和路径）插入到数组中，这样才能打印出来
							   ...
							   string &prefix, string &highlight, string &suffix)
{
	...
	if(pos != string::npos)
	{
		...
		while(str_index < strlower.size())
		{
			...
			{
				//是一个汉字
				string word(strlower, str_index, 2);  //文 一个汉字两个字符
