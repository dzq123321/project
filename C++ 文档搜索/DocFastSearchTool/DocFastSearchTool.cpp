...
{
	const string &path = "E:\\duzhiqiang\\应用信息论";
	//const string &path = "C:\\";
	
	//创建扫描实例
	ScanManager::CreateInstance(path).ScanDirectory(path);//将文件扫描到数据库，这里已经创建了搜索实例

	//创建搜索实例
	DataManager &dm = DataManager::GetInstance();//单例化了搜索实例，所以是一个实例
	...
	while(1)
	{
		...
		if(key == "exit")
			break;
		dm.Search(key, doc_path);//初始化，将数据库的文件导入到doc_path数组中
